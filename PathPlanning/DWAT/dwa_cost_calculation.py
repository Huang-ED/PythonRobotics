...
from dwa_paper_with_width \
    import calc_dynamic_window, closest_obstacle_on_curve, predict_trajectory, calc_to_goal_cost, Config
...
def calculate_all_costs(x, config, goal, ob):
    ...
    for i in range(V.shape[0]):
        for j in range(V.shape[1]):
            ...
            ob_cost[i, j] = max(0., config.max_obstacle_cost_dist-dist)
...
def main():
    log_file_path = "Logs/figs_v7.4.4-vid2_20250701_175055/log_details.csv"
    ...
    # iter_num = 227
    iter_nums = list(range(1335,1345))  # Specify the iterations you want to process
    for iter_num in iter_nums:
        ...
        final_cost = config.to_goal_cost_gain * tg_cost + \
                     config.speed_cost_gain * sp_cost + \
        ...
        plot_cost(axs[3], final_cost, "Final Cost")

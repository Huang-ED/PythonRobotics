...
class Config:
    ...
    def __init__(self):
        ...
        self.port_name = 'sf'
        self.env_image_path = f'env_data/AISData_20240827/land_shapes_{self.port_name}.png'
...
# ----- Set up the map -----
arr = cv2.imread(config.env_image_path, cv2.IMREAD_GRAYSCALE)
...
ob[:, 1] = arr.shape[0] - ob[:, 1]  # Flip y-axis
...
t1 = time.time()
print("A* time: ", time.strftime("%H:%M:%S", time.gmtime(t1 - t0)))
...
print("Done")
t2 = time.time()
print("DWA time: ", time.strftime("%H:%M:%S", time.gmtime(t2 - t1)))
if show_animation:  # pragma: no cover
    plt.show()

...
author: Atsushi Sakai (@Atsushi_twi), Goktuq Karakasli
...
Ablation variant: time decay is disabled in the dynamic obstacle cost.
"""
...
def calc_control_and_trajectory_merged(x, dw, config, goal,
        ...
        for y in np.arange(dw[2], dw[3] + 1e-6, config.yaw_rate_resolution):
            ...
            if has_dynamic:
                ...
                if is_collision:
                    ...
                    # Time decay ablation: equal weight across the horizon.
                    times = np.arange(trajectory_for_obstacle.shape[0]) * config.dt
                    cost_direct_vec = np.ones_like(times, dtype=float)
    ...
    if len(to_goal_costs) == 0:
        ...
        raise ValueError("No admissible (v, omega) pairs found in dynamic window")
...
def calc_trajectory_clearance_and_collision(trajectory, ob_pos, ob_radii, ob_vel, config):
    ...
    # 5. Check Collision and Clearance

...
try:
    # We need specific functions from the updated dwa_weighted_side
    from dwa_weighted_side import (
        Config, calc_dynamic_window, 
        predict_trajectory_to_goal,      # For Goal/Speed costs
        predict_trajectory_obstacle,     # For Obstacle costs
        calc_to_goal_cost, 
    ...
    print("Please ensure 'dwa_weighted_side.py' and 'map_manager.py' are in the same directory.")
    sys.exit(1)
            ...
            if has_dynamic:
                ...
                if is_collision:
                    ...
                    if len(compound_costs) > 0:
                        # Find the critical obstacle (max cost)
...
def main():
    ...
    log_file_path = "Logs/figs_v10.4-video1-newlog_20251205_173431/log_details.csv" 
    map_config_file = "PathPlanning/DWAT_v3_split/map_config/map_config_video1.json"
    
    iter_nums = list(range(595, 600))  # Specify iterations
    # ---------------------

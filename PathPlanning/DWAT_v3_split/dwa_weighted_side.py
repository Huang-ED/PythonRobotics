...
def closest_obstacle_on_side(trajectory, ob, ob_radii, config):
    """
    Find the closest clearance and corresponding trajectory index for all obstacles.
    """
    # Handle empty obstacle list
    if ob is None or ob.shape[0] == 0:
    ...
    if config.robot_type == RobotType.rectangle:
        ...
        if np.any(potential_collisions):
            # Detailed check (same as previous implementation)
            local_ox = ox - traj_x 
            local_oy = oy - traj_y 
    ...
    # Find minimum clearance for each obstacle across the trajectory
    min_clearances = np.min(clearance_matrix, axis=1)
    
    # Find the time index where this minimum occurs
    min_indices = np.argmin(clearance_matrix, axis=1)

    return min_clearances, min_indices, is_collision

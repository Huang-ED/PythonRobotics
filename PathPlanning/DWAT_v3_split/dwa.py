...
def calc_control_and_trajectory_merged(x, dw, config, goal, 
    ...
    for static and dynamic obstacles, and using individual radii.
    ...
    Parameters:
        ...
        static_ob_radii: list of radii for static obstacles [r1, r2, ...]
        dynamic_ob_pos: dynamic obstacle positions [[x(m), y(m)], ...]
        dynamic_ob_radii: list of radii for dynamic obstacles [r1, r2, ...]
    Returns:
...
def closest_obstacle_on_side(trajectory, ob, ob_radii, config):
    ...
    if config.robot_type == RobotType.rectangle:
        ...
        # Note: This rectangle check doesn't account for obstacle radius.
        # It's a simple check if obstacle center is inside robot bounding box.
        if (np.logical_and(np.logical_and(upper_check, right_check),
                           np.logical_and(bottom_check, left_check))).any():
            return 0  # Collision
    elif config.robot_type == RobotType.circle:
        # Check collision for each obstacle with its specific radius
        collision_thresholds = config.robot_radius + ob_radii_np[:, None]
        if (clearance <= collision_thresholds).any():
            return 0 # Collision
    ...
    if config.robot_type == RobotType.circle:
        ...
        clearance = clearance - (config.robot_radius + ob_radii_np[:, None])
    else:
        # For rectangle, clearance is distance from robot center to obstacle edge
        clearance = clearance - ob_radii_np[:, None]

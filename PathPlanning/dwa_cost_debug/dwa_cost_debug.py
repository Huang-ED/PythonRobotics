...
def calculate_all_costs_debug(x, config, goal, ob):
    ...
    for i in range(V.shape[0]):
        for j in range(V.shape[1]):
            ...
            print(dist)
            print(v - math.sqrt(2 * config.max_accel * dist))
            ...
            for _ in range(int(config.check_time / config.dt) + 1):
                # 计算机器人当前位置和碰撞状态
                ...
                # )
                closest_distance, _ = closest_obstacle_on_curve(
                    x_sim.copy(), ob, v, omega, config
                )
                ...
                # 更新状态
                total_dist += v * config.dt / 10
                x_sim = dwa_motion(x_sim, [v, omega], config.dt / 10)

            # 输出最终计算结果
            final_cost_value = 1.0 / collision_dist if collision_dist != 0 else np.inf
            print(
                f"----- 计算结果 [v={v:.2f}, ω={omega:.5f}] -----\n"
                f"到达首次碰撞距离: {collision_dist:.5f}m | "
                f"障碍物成本: {final_cost_value:.5f}\n"
                f"------------------------------------------\n"
            )
...
def main_debug():
    """主调试函数"""
    log_file_path = "Logs/dwa_log_details_20250402_210204/log_details.csv"
    config = Config()

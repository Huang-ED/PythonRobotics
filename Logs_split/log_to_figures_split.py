...
def plot_dwa_logs(log_file_path):
    ...
    # Cost data extraction - UPDATED KEYS
    ...
    static_ob_before = [entry.get('static_ob_cost_before', 0) for entry in log_entries] # Use .get for compatibility
    ...
    static_ob_after = [entry.get('static_ob_cost_after', 0) for entry in log_entries]
    dynamic_ob_after = [entry.get('dynamic_ob_cost_after', 0) for entry in log_entries]
    ...
    # 6. Obstacle Costs (Before Normalization) - UPDATED
    save_plot(iterations, [static_ob_before, dynamic_ob_before],
             'Iteration Number', 'Cost',
             'Obstacle Costs Before Normalization',
             '6. obstacle_costs_before.png',
             labels=['Static Obstacle', 'Dynamic Obstacle'])

    # 7. Normalized Costs (Combined) - UPDATED
    save_plot(iterations, [to_goal_after, speed_after, static_ob_after, dynamic_ob_after],
             'Iteration Number', 'Normalized Cost',
             'Normalized Cost Components',
             '7. costs_after_normalization.png',
             labels=['To Goal', 'Speed', 'Static Obstacle', 'Dynamic Obstacle'])
